...
public class GameCanvas {
    ...
    int height;
    /** The original ratio of the canvas in terms of height / width */
    private float aspectRatio;
    /** The current ratio of the canvas in terms of height / width */
    private float ratio;
    ...
    public GameCanvas() {
        ...
        // Calculate any remaining values needed to be known
        aspectRatio = getHeight() * 1.0f / getWidth();
...
////            camera.viewportWidth = camera.viewportHeight / ratio;
////        } else {
//            camera.viewportHeight = ratio * camera.viewportWidth;
//        }
...
//        spriteBatch.setProjectionMatrix(camera.combined);
    ...
    public void clear(Color color) {
        // Clear the screen
        Gdx.gl.glClearColor(color.r, color.g, color.b, color.a);  // Homage to the XNA years
        Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);
    }
    ...
    /**
     * Start a standard drawing sequence scaled to the current window size.
     *
     * Nothing is flushed to the graphics card until the method end() is called.
     */
    public void beginScaled() {
        global.idt();
        if (ratio > aspectRatio) {
            global.scl(ratio / aspectRatio, 1, 1.0f);
        } else {
            global.scl(1, 1 / ratio * aspectRatio, 1.0f);
        }
        global.mulLeft(camera.combined);
    ...
    /**
     * Draws text centered at the given x, y coordinates.
     *
     * @param text The string to draw
     * @param font The font to use
    ...
    /**
     * Return the current value that the x-axis is being scaled by
     *
     * @return current value that the x-axis is being scaled by
     */
    public float getScaleX() {
        return ratio > aspectRatio ? ratio / aspectRatio : 1;
    }

    /**
     * Return the current value that the y-axis is being scaled by
     *
     * @return current value that the y-axis is being scaled by
     */
    public float getScaleY() {
        return ratio > aspectRatio ? 1 : aspectRatio / ratio;
    }
}
